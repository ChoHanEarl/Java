	...
	public static void main(String[] args) {
		...
		if(f.exists()) {
			try {
				...
				//read()메서드는 텍스트를 읽어올 때 1byte씩밖에 읽어올 수 없기 때문에
				//한글 데이터를 읽는 게 불가능하다.(한글은 2 ~ 3byte이기 때문에)
				while((code = fis.read()) != -1) {
					//더 이상 읽어올 단어가 없으면 문장의 끝(End Of File)인 -1을 반환한다.
					//아스키코드, 유니코드 어느곳에서도 -1의 의미를 갖고 있는 것은 없기 때문이다.
					System.out.print((char)code); //¾È³çÇÏ¼¼¿ä -> 안녕하세요
				}
				
				//스트림은 사용이 완료된 이후 close를 통해 닫아주는 것이 좋다.
				//그래야 다음 작업 시 문제가 생기지 않는다.
